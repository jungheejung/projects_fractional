#!/usr/bin/env python
"""Builds a glm based on theory of mind Spunt task task
...
contrast of interest: check how to model the regressors
"""
...
spunt_flist = glob.glob(join(beh_dir, '**', f'*{task_name}*.tsv'), recursive=True)
print(main_dir)
print(spunt_flist)
spunt_behfpath = spunt_flist[slurm_id]
spunt_behfname = os.path.basename(spunt_behfpath)

#   -> extract sub, ses, task , run 
sub = re.search(r'sub-\d+', spunt_behfname).group(0)
ses = re.search(r'ses-\d+', spunt_behfname).group(0)
run = re.search(r'run-\d+', spunt_behfname).group(0)
run_num = int(re.search(r'run-(\d+)', spunt_behfname).group(1))
...
# ------------------------------------
#   -> load events.tsv:
events_fname = spunt_behfpath #join(beh_dir, f"{sub}_{ses}_task-{task_name}_{run}_events.tsv")
...
plt.plot(design_matrix["how_face"]); plt.xlabel("scan"); plt.title("how_face conditions")
...
conditions = {"buttonpress": np.zeros(design_matrix.shape[1]), 
              "how_face": np.zeros(design_matrix.shape[1]),
              "how_hand": np.zeros(design_matrix.shape[1]),
              "why_face": np.zeros(design_matrix.shape[1]),
              "why_hand": np.zeros(design_matrix.shape[1])}
conditions["buttonpress"][0] = 1
conditions["how_face"][1] = 1
conditions["how_hand"][2] = 1
conditions["why_face"][3] = 1
...
plotting.plot_contrast_matrix(why_gt_how, design_matrix=design_matrix)
...
plt.savefig(join(glm_savedir, f"{sub}_{ses}_task-{task_name}_run-{run_num:02d}_con-01_desc-whyGThow_stat-zmap.png")) 
plt.close() 
