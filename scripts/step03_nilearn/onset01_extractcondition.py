...
beh_inputdir = join(main_dir, 'data/dartmouth/d02_preprocessed')
beh_savedir = join(main_dir, 'data' , 'beh', 'beh03_bids', sub)
Path(beh_savedir).mkdir( parents=True, exist_ok=True )
beh_fpath = join(beh_inputdir, sub, 'sub-0009_ses-04_task-fractional_run-02-tomsaxe_beh.csv')
...
string = os.path.basename(beh_fpath)
sub_bids = re.search(r'sub-\d+', string).group(0)
ses_bids = re.search(r'ses-\d+', string).group(0)
run_bids = re.search(r'run-\d+', string).group(0)
task_name = re.search(r'run-\d+-(\w+)_beh', string).group(1)
